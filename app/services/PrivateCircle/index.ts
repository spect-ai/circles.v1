...
export const createPrivateCircleCredentials = async (
  circleCreate: Partial<CreatePrivateCirclePropertiesDto>,
  circleId: string
) => {
  const res = await fetch(`${process.env.API_HOST}/circle/private/v1/`, {
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
    },
    method: "POST",
    body: JSON.stringify(circleCreate),
    credentials: "include",
  });
  if (res.ok) {
