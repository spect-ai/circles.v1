...
const ViewEducationModal = ({
  ...
  if (education && (educationId || educationId === 0)) {
    return (
      <Modal
        handleClose={() => {
          handleClose();
      ...
      >
        ...
        >
          ...
          <ScrollContainer>
            <Box>
              <Editor value={education.description} disabled />
            </Box>
            {education.linkedCredentials?.length > 0 && (
              <Box>
                <Text variant="label">Linked Credentials</Text>
                <Box marginTop="4">
                  {education.linkedCredentials?.map((credential) => {
...
ViewEducationModal.defaultProps = {
  educationId: null,
};
